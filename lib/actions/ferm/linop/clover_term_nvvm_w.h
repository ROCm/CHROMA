...
#ifndef __clover_term_nvvm_w_h__
#define __clover_term_nvvm_w_h__

#warning "Using QDP-JIT/NVVM clover"
...
{ 
  ...
  class NVVMCloverTermT : public CloverTermBase<T, U>
  {
  public:
  ...
  private:
   ...
   // Empty constructor. Must use create later
  template<typename T, typename U>
  NVVMCloverTermT<T,U>::NVVMCloverTermT() {}

  // Now copy
  template<typename T, typename U>
  void NVVMCloverTermT<T,U>::create(Handle< FermState<T,multi1d<U>,multi1d<U> > > fs,
				   const CloverFermActParams& param_,
				   const NVVMCloverTermT<T,U>& from)
  {
    START_CODE();
    ...
    // Sanity check
    if (fbc.operator->() == 0) {
      QDPIO::cerr << "NVVMCloverTerm: error: fbc is null" << std::endl;
      QDP_abort(1);
    }
  ...
  void function_make_clov_exec(hipFunction_t function, 
  ...
  template<typename RealT,typename U,typename X,typename Y>
  hipFunction_t function_make_clov_build(const RealT& diag_mass, 
				      const U& f0,
				      const U& f1,
				      const U& f2,
				      const U& f3,
				      ...
				      const Y& tri_off)
  {
    ...
    if (ptx_db::db_enabled) {
      hipFunction_t func = llvm_ptx_db( __PRETTY_FUNCTION__ );
      if (func)
	return func;
    }
  ...
  template<typename T, typename U>
  void NVVMCloverTermT<T,U>::makeClov(const multi1d<U>& f, const RealT& diag_mass)
  {
    START_CODE();
   ...
   */
  template<typename T, typename U>
  void NVVMCloverTermT<T,U>::choles(int cb)
  {
    START_CODE();
   ...
   */
  template<typename T, typename U>
  Double NVVMCloverTermT<T,U>::cholesDet(int cb) const
  {
    START_CODE();
  ...
  void function_ldagdlinv_exec( hipFunction_t function,
				      T& tr_log_diag,
				      X& tri_dia,
				      Y& tri_off,
				      const Subset& s)
  ...
  hipFunction_t function_ldagdlinv_build( const T& tr_log_diag,
				       const X& tri_dia,
				       const Y& tri_off,
				       const Subset& s)
  {
  ...
  template<typename T, typename U>
  void NVVMCloverTermT<T,U>::ldagdlinv(LatticeREAL& tr_log_diag, int cb)
  {
    START_CODE();
  ...
  void function_triacntr_exec( hipFunction_t function,
				     U& B,
				     const X& tri_dia,
				     const Y& tri_off,
				     int mat,
  ...
  template<typename U,typename X,typename Y>
  hipFunction_t function_triacntr_build( const U& B,
				      const X& tri_dia,
				      const Y& tri_off,
				      int mat,
				      const Subset& s)
  ...
  template<typename T, typename U>
  void NVVMCloverTermT<T,U>::triacntr(U& B, int mat, int cb) const
  {
    START_CODE();
  ...
  template<typename T, typename U>
  Real
  NVVMCloverTermT<T,U>::getCloverCoeff(int mu, int nu) const 
  { 
    START_CODE();
  ...
  template<typename T,typename X,typename Y>
  void function_apply_clov_exec(hipFunction_t function,
				T& chi,
				const T& psi,
				const X& tri_dia,
				const Y& tri_off,
  ...
  template<typename T,typename X,typename Y>
  hipFunction_t function_apply_clov_build(const T& chi,
				       const T& psi,
				       const X& tri_dia,
				       const Y& tri_off,
				       const Subset& s)
   ...
   */
  template<typename T, typename U>
  void NVVMCloverTermT<T,U>::apply(T& chi, const T& psi, 
  ...
  template<typename T, typename U>
  void NVVMCloverTermT<T,U>::packForQUDA(multi1d<QUDAPackedClovSite<typename WordType<T>::Type_t> >& quda_array, int cb) const
    {
      typedef typename WordType<T>::Type_t REALT;
  ...
  template<typename T, typename U>
  void NVVMCloverTermT<T,U>::applySite(T& chi, const T& psi, 
				      enum PlusMinus isign, int site) const
  {
    QDP_error_exit("NVVMCloverTermT<T,U>::applySite(T& chi, const T& psi,..) not implemented ");
#if 0
#endif
  }

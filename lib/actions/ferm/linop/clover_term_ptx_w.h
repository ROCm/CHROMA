...
#warning "Using QPD-JIT/PTX clover"
...
{ 
				   ...
				   const PTXCloverTermT<T,U>& from)
  {
    ...
    // Sanity check
    if (fbc.operator->() == 0) {
      QDPIO::cerr << "PTXCloverTerm: error: fbc is null" << std::endl;
      QDP_abort(1);
    }
  ...
  hipFunction_t function_make_clov_exec(hipFunction_t function, 
  ...
  hipFunction_t function_ldagdlinv_exec( hipFunction_t function,
				      T& tr_log_diag,
				      X& tri_dia,
				      Y& tri_off,
				      const Subset& s)
  ...
  hipFunction_t function_triacntr_exec( hipFunction_t function,
				     U& B,
				     const X& tri_dia,
				     const Y& tri_off,
				     int mat,
